...
Creates its own listings through the admin API (X-User-Role is what the gateway injects).
"""
import uuid
...
def listings(client: TestClient) -> dict:
    created = {}
    for key, payload in {
        "condo": _listing(),
        ...
        r = client.post("/api/v1/houses", json=payload, headers=ADMIN)
        assert r.status_code == 200, r.text
        created[key] = r.json()
    return created


def test_write_requires_admin_role(client: TestClient) -> None:
    assert client.post("/api/v1/houses", json=_listing()).status_code == 403
    user = {"X-User-ID": "2", "X-User-Role": "user"}
    assert client.post("/api/v1/houses", json=_listing(), headers=user).status_code == 403
    assert client.put("/api/v1/houses/1", json={"price": 1}, headers=user).status_code == 403
    assert client.delete("/api/v1/houses/1", headers=user).status_code == 403
...
def test_price_change_is_recorded_and_filterable(client: TestClient, listings) -> None:
    house_id = listings["detached"]["id"]
    r = client.put(f"/api/v1/houses/{house_id}", json={"price": 1425000}, headers=ADMIN)
    assert r.status_code == 200
