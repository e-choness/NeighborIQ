    ...
    for obs in payload.get("observations", []):
        ...
        for series, cell in obs.items():
            ...
            if day and value is not None:
                rows.append({
                    "series": f"boc:{series}", "date": day, "value": value,
...
def load_statcan_table(session: Session, fh, series_prefix: str, filters: dict[str, str],
                       geo_column: str = "GEO", source: str = "statcan") -> int:
    """
    ...
    for row in reader:
        ...
        day = to_date((row.get("REF_DATE") or "") + "-01" if len(row.get("REF_DATE", "")) == 7 else row.get("REF_DATE"))
        if value is None or day is None:
            continue
        geo = row.get(geo_column, "").split(",")[0].strip()
        rows.append({
            "series": f"{series_prefix}:{geo.lower()}"[:64], "date": day, "value": value,
            "label": f"{series_prefix} — {geo}"[:255], "unit": row.get("UOM", "")[:32], "source": source,
        })
        if len(rows) >= 5000:
            session.execute(_UPSERT, rows)
            rows = []
