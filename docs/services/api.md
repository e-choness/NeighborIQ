...
- RS256 JWTs (`app/tokens.py`; passwords hashed with Argon2id in `app/passwords.py`; bcrypt hashes from earlier releases are accepted and upgraded at the next sign-in). The access token lasts 15 minutes, the refresh token 7 days and is rotated on every use, and
